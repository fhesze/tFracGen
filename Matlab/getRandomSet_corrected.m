...
%         Nu(:,2) = r.*sin(2.*pi.*Gamma(:,1))./lMaxY;
        Nu(:,1) = randn(params.nMode, 1)./(lMaxX/sqrt(2));
        Nu(:,2) = randn(params.nMode, 1)./(lMaxY/sqrt(2));
    case 'Exp'
...
switch params.func
    case 'Gauss'
        Nu(:,1) = randn(n0, 1)./(lMax(1)/sqrt(2));
        Nu(:,2) = randn(n0, 1)./(lMax(2)/sqrt(2));
        Nu(:,3) = randn(n0, 1)./(lMax(3)/sqrt(2));
    otherwise
        fii     = 2*pi*Gamma(:,2);
        th      = acos(1 - 2*Gamma(:,3));
